...
- **Critical**: Fixed n8n v1.x compatibility - added fallback for `NodeConnectionType` vs `NodeConnectionTypes`
  - Package now works with both n8n v1.x (using `NodeConnectionType`) and v2.x (using `NodeConnectionTypes`)
  - Resolves "Class could not be found" error when installing via n8n UI on instances with mixed n8n-workflow versions
...
- Added `strict: true` to n8n configuration section (matching official n8n-nodes-starter template)
- Reordered n8n config fields to match official template structure
